...
class RbinterPipeline:

    # Database setup
    conn = sqlite3.connect('rbinter.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `rbinter`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM rbinter WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO rbinter (date, title, link, content)"
